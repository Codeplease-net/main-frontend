    ...
    return (
        <header className="border-b relative bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
            <nav className="mx-auto flex justify-between items-center h-20 px-4">
                <Link href="/" className="text-2xl font-bold flex items-center hover:opacity-90 transition-opacity">
                    <Image src={Lightmode ? logoheadwhite : logoheadblack} height={48} alt="logo-head" priority />
                </Link>
                ...
                <div className="flex items-center gap-3">
                    <div className="hidden md:flex items-center gap-3">
                        <LocaleSwitcher />
                        <Button variant="outline" size="icon" onClick={() => setLightmode(!Lightmode)} className="hover:bg-secondary/80">
                        ...
                        {user ? (
                            <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                    <DropdownMenuLabel className="font-medium">{isLoadingHandle ? "Loading..." : userHandle || "User"}</DropdownMenuLabel>
                    ...
                    <div className="flex flex-col p-4 space-y-3">
                        {["home", "problems", "leaderboard", "topics"].map((route, index) => (
                            <Link
                                ...
                                className={`px-4 py-2 rounded-md transition-colors ${
                                    pathName.includes(route) ? "bg-secondary text-primary" : "hover:bg-secondary/80"
                                }`}
                            >
                                {t(route.charAt(0).toUpperCase() + route.slice(1))}
                            </Link>
                        ...
                        {user ? (
                            <DropdownMenu>
                                ...
                                <DropdownMenuContent>
                                    <DropdownMenuLabel>{isLoadingHandle ? "Loading..." : userHandle || "User"}</DropdownMenuLabel>
